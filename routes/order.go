...
func CreateOrder(c *fiber.Ctx) error {
	...
	connectdb.Database.Db.Create(&order)
...
func GetOrders(c *fiber.Ctx) error {
	orders := []models.Order{}
	connectdb.Database.Db.Find(&orders)
	responseOrders := []Order{}

	for _, order := range orders {
		var user models.User
		var product models.Product
		connectdb.Database.Db.Find(&user, "id = ?", order.UserRefer)
		connectdb.Database.Db.Find(&product, "id = ?", order.ProductRefer)
...
func findOrder(id int, order *models.Order) error {
	connectdb.Database.Db.Find(&order, "id = ?", id)
	if order.ID == 0 {
